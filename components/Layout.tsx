...
const Layout: React.FC<LayoutProps> = ({
  ...
  const isCommandCenterActive = NAV_ITEMS.some(
    (item) =>
      commandCenterIds.includes(item.id) &&
  ...
  const [isCommandCenterOpen, setIsCommandCenterOpen] = useState(
  ...
  const criticalUnreadCount = unreadNotifications.filter(
    (n) => n.type === "critical" || n.type === "overdue"
  ).length;
  ...
  useEffect(() => {
    if (unreadCount > prevUnreadCountRef.current) {
      ...
      const isUrgent =
        latest?.type === "critical" || latest?.type === "overdue";
  ...
  const filteredNavItems = NAV_ITEMS.filter((item) =>
    user.allowedModules.includes(item.moduleId)
  );
  ...
  const getNotifIcon = (type: string) => {
    switch (type) {
      ...
      default:
        return <BellRing className="text-indigo-500" size={16} />;
    }
  };
  ...
  const getNotifColor = (type: string, isRead: boolean) => {
    ...
    switch (type) {
      case "critical":
        return "bg-red-50 border-l-4 border-red-500 dark:bg-red-950/20";
      case "overdue":
        return "bg-orange-50 border-l-4 border-orange-500 dark:bg-orange-950/20";
      default:
        return "bg-blue-50 border-l-4 border-blue-500 dark:bg-blue-950/20";
    }
  };
    ...
    <div className="h-screen w-full flex bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans overflow-hidden">
      ...
      {activeToast && (
        <div className="fixed top-24 right-6 z-[200] w-80 bg-slate-900/95 dark:bg-slate-800/95 backdrop-blur-md text-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-slate-700 overflow-hidden animate-slide-in-right">
          <div
            className={`h-1 animate-toast-progress ${
              activeToast.type === "critical" ? "bg-red-500" : "bg-blue-500"
            }`}
          ></div>
          <div className="p-4 flex gap-4">
            <div
              className={`p-2.5 rounded-xl shrink-0 h-fit ${
      ...
      >
        ...
        <div className="h-20 flex items-center px-6 border-b border-slate-800 bg-slate-950 shrink-0">
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-slate-500 tracking-widest uppercase">
              Project Information System IFM
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
          {(() => {
            const dashboardItem = filteredNavItems.find(
              (item) => item.id === "dashboard"
            );
            const commandCenterItems = filteredNavItems.filter((item) =>
              commandCenterIds.includes(item.id)
            );
            const otherItems = filteredNavItems.filter(
              (item) =>
                !commandCenterIds.includes(item.id) && item.id !== "dashboard"
            );
              ...
              <>
                ...
                {dashboardItem && (
                  <div className="space-y-1">
                    <div
                      className={`w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-all duration-200 group relative cursor-pointer ${
                        isDashboardActive
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative">
              <button
                ...
                className={`p-2.5 rounded-xl transition-all hover:bg-gray-100 dark:hover:bg-slate-800 relative ${
                  unreadCount > 0
                  ...
                  criticalUnreadCount > 0
                    ? "animate-bell-shake text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950 ring-red-500 shadow-[0_0_15px_rgba(239,68,68,0.4)]"
                    : ""
                }`}
              >
                <Bell size={22} />
                  ...
                  <span
                    className={`absolute top-0 right-0 w-5 h-5 ${
                      criticalUnreadCount > 0
                        ...
                        : "bg-blue-600"
                    } text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900 transform translate-x-1 -translate-y-1 shadow-md`}
                  >
                    {unreadCount}
                  </span>
                )}
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                      {notifications.length === 0 ? (
                        <div className="p-10 text-center flex flex-col items-center">
                          <BellRing
                            className="text-gray-200 dark:text-slate-800 mb-2"
                            size={32}
                          />
                        ...
                        notifications.map((notif) => (
                          <div
                            key={notif.id}
                            className={`p-4 border-b border-gray-50 dark:border-slate-800 transition-colors relative group ${getNotifColor(
                              notif.type,
                              notif.read
                            )}`}
                          >
          ...
          <div className="bg-red-600 text-white px-6 py-3 flex items-center justify-between animate-banner-pulse shrink-0 shadow-lg relative z-[5]">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-red-100 font-bold opacity-90">
                  {criticalUnreadCount} unread high-priority safety or project
                  tasks require immediate verification.
                </p>
              </div>
            </div>
            <button
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full ${
                      event.type === "Deadline" ? "bg-red-500" : "bg-blue-400"
                    }`}
                  ></span>
      ...
      {showFeedbackModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-slate-900/60 dark:bg-black/80 backdrop-blur-md animate-fadeIn"
            ...
            {feedbackSuccess ? (
              <div className="p-12 text-center animate-fadeIn">
                ...
                <p className="text-gray-500 dark:text-slate-400">
                  Your feedback helps us build a better PISIFM experience.
                </p>
              </div>
            ) : (
              <div className="p-8">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-gray-500 dark:text-slate-400 text-sm">
                      How is your experience with the system?
                    </p>
                  </div>
                  <button
                ...
                <form onSubmit={handleFeedbackSubmit} className="space-y-6">
                        ...
                        return (
                          ...
                          >
                            <RatingIcon
                              size={28}
                              className={
                                isSelected ? rating.color : "text-gray-400"
                              }
                            />
                            <span
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <option value="New Feature" className="dark:bg-slate-900">
                        New Feature Request
                      </option>
                      <option value="Bug Report" className="dark:bg-slate-900">
                        Technical Bug / Error
                      </option>
                      <option value="Performance" className="dark:bg-slate-900">
                        Performance / Speed
                      </option>
            ...
            <div className="px-8 pb-8">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-700">
                  ...
                  <p className="font-mono text-sm text-gray-800 dark:text-slate-100">
                    {user.id}
                  </p>
                </div>
                <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-2xl border border-gray-100 dark:border-slate-800">
                  ...
                  <p className="text-sm text-gray-800 dark:text-slate-100 font-bold">
                    {user.role}
                  </p>
                </div>
              </div>
