...
import { User, Submission, FormType, ApprovalFlow, JobAssignment, Project } from '../types';
import { DEFAULT_APPROVAL_FLOWS } from '../constants';
import {
    FileText, Clock, ShoppingCart, LogOut,
    Camera, Send, Loader2,
    TrendingUp, BarChart3, Info,
    Timer, Calendar, AlertCircle, CheckCircle, X,
    ShieldCheck, ArrowRight, Eye, ClipboardList,
    Layers
} from 'lucide-react';
...
const FormsView: React.FC<FormsViewProps> = ({ user, submissions, jobs, projects, onAddSubmission }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const formDetails = activeForm === 'overtime' ? overtime :
            activeForm === 'daily_report' ? dailyReport :
                activeForm === 'leave' ? leaveRequest :
                    activeForm === 'purchase' ? purchaseRequest : lateArrival;
    ...
    const getFormTitle = (type: FormType) => {
        switch (type) {
            case 'daily_report': return 'Daily Activity Report';
            case 'leave': return 'Leave Permit Request';
            case 'purchase': return 'Purchase Requisition';
            case 'overtime': return 'Overtime Allocation';
            case 'late_arrival': return 'Tardiness Notice';
    ...
    const TabButton = ({ id, label, icon: Icon }: any) => (
        <button
            ...
            className={`flex items-center gap-3 px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeForm === id
        ...
        <div className="space-y-10 animate-fadeIn">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-slate-400 mt-2 font-medium">Standardized digital reporting and requisition forms.</p>
                </div>
            </div>
                ...
                <div className="space-y-8">
                    <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800 p-8 transition-all">
                        ...
                        <div className="space-y-6">
                            {submissions.map((sub) => (
                                <div key={sub.id} className="p-5 rounded-2xl hover:bg-gray-50 dark:hover:bg-slate-800 transition-all border border-transparent hover:border-gray-100 dark:hover:border-slate-700">
                                    <div className="flex items-center gap-4 mb-4">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${sub.status === 'Approved' ? 'bg-emerald-50 text-emerald-600' :
                                            sub.status === 'Rejected' ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'
                                            }`}>
                                            <FileText size={20} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="font-black text-gray-800 dark:text-slate-200 text-sm truncate uppercase tracking-tight">{sub.title}</p>
                                            <p className="text-[10px] font-bold text-gray-400">{sub.date} • ID: {sub.id}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-center px-1">
                                            ...
                                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border ${sub.status === 'Approved' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                                                sub.status === 'Rejected' ? 'bg-rose-50 text-rose-600 border-rose-100' : 'bg-amber-50 text-amber-700 border-amber-100'
                                                }`}>{sub.status}</span>
                                        </div>
                                        <div className="w-full h-1.5 bg-gray-100 dark:bg-slate-700 rounded-full overflow-hidden">
                                            <div className={`h-full transition-all duration-1000 ${sub.status === 'Approved' ? 'bg-emerald-500 w-full' : sub.status === 'Rejected' ? 'bg-rose-500 w-full' : 'bg-amber-500 w-1/3'}`}></div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="bg-indigo-600 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden">
                        ...
                        <h3 className="text-lg font-black uppercase tracking-widest mb-4">Workflow Guard</h3>
                        <p className="text-sm font-medium opacity-80 leading-relaxed mb-6">
                            Documents require <span className="font-black underline">Multi-Tier Validation</span>. Review status live in this panel for real-time verification of departmental clearance.
                        </p>
