...
const ProjectsView: React.FC<ProjectsViewProps> = ({ currentUser, projects, users, onNavigate, onRefresh }) => {
    ...
    const projectsWithUserNames = projects.map(project => ({
        ...project,
        assignee: users.find(u => u.id === project.assignee)?.name || project.assignee
    }));
    ...
    const filteredProjects = projectsWithUserNames.filter(p =>
        ...
        <div className="h-screen flex flex-col bg-slate-50 dark:bg-slate-950">
            ...
            <div className="px-8 py-6 flex flex-col gap-6 shrink-0">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Projects</h1>
                        <p className="text-slate-500 dark:text-slate-400 mt-1">Manage, track, and collaborate on engineering projects.</p>
                    </div>
                    <button
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex bg-white dark:bg-slate-900 p-1 rounded-xl border border-gray-200 dark:border-slate-800">
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'board' ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <LayoutGrid size={18} />
                        </button>
                        <button
                            onClick={() => setViewMode('list')}
                            className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <ListIcon size={18} />
                        </button>
                    </div>
