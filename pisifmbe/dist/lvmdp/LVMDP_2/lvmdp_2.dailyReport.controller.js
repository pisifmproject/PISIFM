...
router.get("/all", async (req, res) => {
    try {
        ...
        const { saveDailyReport } = await Promise.resolve().then(() => __importStar(require("./lvmdp_2.dailyReport.repository")));
...
router.get("/hourly/:date", async (req, res) => {
    try {
        ...
        if (data.length === 0 || isToday) {
            ...
            const { generateHourlyReportsFromView } = await Promise.resolve().then(() => __importStar(require("./lvmdp_2.hourlyReport.services")));
