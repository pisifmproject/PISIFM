...
router.get("/all", async (req, res) => {
    try {
        const t0 = Date.now();
        const { getAllDailyReports } = await Promise.resolve().then(() => __importStar(require("./lvmdp_3.dailyReport.repository")));
        const { saveDailyReport } = await Promise.resolve().then(() => __importStar(require("./lvmdp_3.dailyReport.repository")));
        ...
        const t1 = Date.now();
        console.log(`[LVMDP3 Daily /all] Fetched ${formatted.length} reports in ${t1 - t0}ms`);
        return res.json({
            success: true,
            data: formatted,
        });
...
router.get("/hourly/:date", async (req, res) => {
    try {
        ...
        if (data.length === 0 || isToday) {
            ...
            const { generateHourlyReportsFromView } = await Promise.resolve().then(() => __importStar(require("./lvmdp_3.hourlyReport.services")));
