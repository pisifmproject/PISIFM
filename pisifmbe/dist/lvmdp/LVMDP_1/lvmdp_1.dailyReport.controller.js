...
router.get("/all", async (req, res) => {
    try {
        ...
        const { getAllDailyReports } = await Promise.resolve().then(() => __importStar(require("./lvmdp_1.dailyReport.repository")));
        const { saveDailyReport } = await Promise.resolve().then(() => __importStar(require("./lvmdp_1.dailyReport.repository")));
        ...
        const todayExists = allReports.some((r) => r.reportDate === today);
        if (!todayExists) {
        ...
        const t1 = Date.now();
        console.log(`[LVMDP1 Daily /all] Fetched ${formatted.length} reports in ${t1 - t0}ms`);
        return res.json({
            success: true,
            data: formatted,
        });
...
router.get("/hourly/:date", async (req, res) => {
    try {
        ...
        if (data.length === 0 || isToday) {
            ...
            const { generateHourlyReportsFromView } = await Promise.resolve().then(() => __importStar(require("./lvmdp_1.hourlyReport.services")));
