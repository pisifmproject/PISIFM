...
router.get("/hourly/:date", async (req, res) => {
    try {
        ...
        const { getDailyReportByDate } = await Promise.resolve().then(() => __importStar(require("./lvmdp_1.dailyReport.repository")));
        const existing = await getDailyReportByDate(new Date(date));
        if (!existing || existing.length === 0) {
            try {
                await (0, lvmdp_1_dailyReport_services_1.generateAndSaveDailyReport)(date);
            }
            catch (genErr) {
                // lanjut, tidak perlu error
            }
        }
        const rows = await (0, lvmdp_1_dailyReport_services_1.fetchHourlyAggregates)(date);
