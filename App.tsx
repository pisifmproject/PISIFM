...
const FinancialReportsView = React.lazy(() => import('./components/FinancialReportsView'));
const ResourcePlanningView = React.lazy(() => import('./components/ResourcePlanningView'));
...
const PlaceholderPage = ({ title }: { title: string }) => (
  <div className="flex flex-col items-center justify-center h-96 text-center p-8 bg-white dark:bg-slate-900 border border-dashed border-gray-300 dark:border-slate-800 rounded-xl">
    <h3 className="text-2xl font-bold text-gray-300 dark:text-slate-700 mb-2">{title}</h3>
    <p className="text-gray-400 dark:text-slate-500">This module is part of the architecture but not active in this demo.</p>
  </div>
);
...
import { projectService, userService, jobService, submissionService } from './services/api';
...
const App: React.FC = () => {
  ...
  const currentUser = users.find(u => u.id === currentUserId) || users[0] || MOCK_USERS_DB[0];

  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [projectsData, usersData, jobsData, submissionsData] = await Promise.all([
        ...
        setSubmissions(submissionsData.length > 0 ? submissionsData : MOCK_SUBMISSIONS);
      } catch (error) {
  ...
  const handleUpdateUserModules = async (userId: string, moduleId: string) => {
    try {
      ...
      setUsers(prev => prev.map(u => u.id === userId ? { ...u, allowedModules: newModules } : u));
    } catch (error) {
      console.error('Error updating user modules:', error);
    }
  };

  const handleUpdateUserHierarchy = async (userId: string, updates: Partial<User>) => {
    try {
          ...
          !isAuthenticated ? <Navigate to="/login" /> :
            ...
            >
              <Suspense fallback={<LoadingSpinner />}>
                <Routes>
                  <Route path="/" element={<DashboardView user={currentUser} users={users} jobs={jobs} projects={projects} onNavigate={(path) => window.location.hash = path} />} />
                  <Route path="/ai-copilot" element={<AICopilotView user={currentUser} projects={projects} />} />
                  <Route path="/finance" element={<FinancialReportsView user={currentUser} projects={projects} />} />
                  <Route path="/projects" element={<ProjectsView currentUser={currentUser} users={users} projects={projects} onNavigate={(path) => window.location.hash = path} onRefresh={async () => setProjects(await projectService.getAll())} />} />
                  <Route path="/performance" element={<KPICalculator user={currentUser} submissions={submissions} />} />
                  <Route path="/requests" element={<FormsView user={currentUser} submissions={submissions} jobs={jobs} projects={projects} onAddSubmission={handleAddSubmission} />} />
                  <Route path="/resources" element={<ResourcePlanningView currentUser={currentUser} projects={projects} users={users} />} />
                  <Route path="/admin" element={<UserManagementView currentUser={currentUser} users={users} flows={approvalFlows} onUpdateFlows={setApprovalFlows} onUpdateUserModules={handleUpdateUserModules} onUpdateUserRole={handleUpdateUserRole} onUpdateHierarchy={handleUpdateUserHierarchy} onUserSelect={setCurrentUserId} />} />
                  <Route path="/safety" element={<PlaceholderPage title="Module 24: EHS & Safety" />} />
