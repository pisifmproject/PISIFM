  ...
  try {
    const result = await pool.query(
      ...
      [
        ...
        JSON.stringify(plannedProgress || []),
        JSON.stringify(actualProgress || []),
...
app.post("/api/submissions", async (req, res) => {
  const { id, type, title, status, details, flowId, currentStepIndex, userId } =
    req.body;
  try {
    const result = await pool.query(
...
app.get("/api/jobs", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM job_assignments ORDER BY created_at DESC"
    );
...
app.put("/api/projects/:id", async (req, res) => {
  ...
  const values = keys.map((key) =>
    typeof fields[key] === "object" ? JSON.stringify(fields[key]) : fields[key]
  );
  try {
    const result = await pool.query(
